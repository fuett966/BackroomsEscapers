...
{
    public class HandsHolder : NetworkBehaviour
    {
        [Header("HandsHolder")]
        [SerializeField]
        bool Enabled = true;
        ...
        float SprintAmount = 1.4f;

        [SerializeField, Range(5f, 20f)]
        float Frequency = 13.0f;
        ...
        [SerializeField]
        bool EnabledRotationMovement = true;

        [SerializeField, Range(0.1f, 10.0f)]
        float RotationMultipler = 6f;
        ...
        public override void OnStartClient()
        {
            ...
            if (player.transform.GetComponent<PlayerController>() != null)
                ToggleSpeed = player.transform.GetComponent<PlayerController>().CroughSpeed * 1.5f;
            else
                ToggleSpeed = 1.5f;
            AmountValue = Amount;
        ...
        private void Update()
        {
            ...
            if (speed > ToggleSpeed && player.isGrounded)
            {
                ...
                FinalRot +=
                    new Vector3(-HeadBobMotion().z, 0, HeadBobMotion().x) * RotationMultipler * 10;
            }
            ...
            if (EnabledRotationMovement)
                transform.localRotation = Quaternion.Lerp(
                    transform.localRotation,
                    Quaternion.Euler(FinalRot),
                    Smooth / 1.5f * Time.deltaTime
                );
        }
        ...
        private Vector3 HeadBobMotion()
        {
            ...
            pos.y += Mathf.Lerp(
                pos.y,
                Mathf.Sin(Time.time * Frequency) * AmountValue * 2f,
                Smooth * Time.deltaTime
            );
            pos.x += Mathf.Lerp(
                pos.x,
                Mathf.Cos(Time.time * Frequency / 2f) * AmountValue * 1.3f,
                Smooth * Time.deltaTime
            );
            return pos;
        }
